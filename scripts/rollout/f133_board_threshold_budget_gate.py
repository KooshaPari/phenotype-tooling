...
def fail(message: str) -> None:
    print(f"E133 board threshold budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    for index, record in enumerate(records):
        ...
        if threshold_budget_spent > args.max_threshold_budget_spent:
            fail(
                f"{args.threshold_budget_spent_field}={threshold_budget_spent} > "
                f"{args.max_threshold_budget_spent} at index {index}"
            )

        over_threshold_budget_count = to_int(
            record.get(args.over_threshold_budget_count_field),
            args.over_threshold_budget_count_field,
            index,
        )
        if over_threshold_budget_count > args.max_over_threshold_budget_count:
            fail(
                f"{args.over_threshold_budget_count_field}={over_threshold_budget_count} > "
                f"{args.max_over_threshold_budget_count} at index {index}"
            )
