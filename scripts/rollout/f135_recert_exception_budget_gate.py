...
def fail(message: str) -> None:
    print(f"E135 recert exception budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    for index, record in enumerate(records):
        exception_budget_spent = to_float(
            record.get(args.exception_budget_spent_field), args.exception_budget_spent_field, index
        )
        ...
        if over_exception_budget_count > args.max_over_exception_budget_count:
            fail(
                f"{args.over_exception_budget_count_field}={over_exception_budget_count} > "
