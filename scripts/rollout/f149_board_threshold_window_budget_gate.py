...
def fail(message: str) -> None:
    print(f"E149 board threshold window budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument(
        "--threshold-window-budget-spent-field",
        default="threshold_window_budget_spent",
    )
    ...
    for index, record in enumerate(records):
        ...
        if threshold_window_budget_spent > args.max_threshold_window_budget_spent:
            fail(
                f"{args.threshold_window_budget_spent_field}="
                f"{threshold_window_budget_spent} > "
