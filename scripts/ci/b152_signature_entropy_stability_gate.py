...
def fail(message: str) -> None:
    print(
        f"E152 [lane B] signature entropy stability gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    for row in records:
        ...
        if instability_events > samples:
            fail(f"instability_events={instability_events} cannot exceed samples={samples}")
        ...
        window_weighted_entropy[window] = (
            window_weighted_entropy.get(window, 0.0) + (entropy_score * samples)
        )
    ...
    average_stability_score = weighted_stability_sum / total_samples
    ...
    total_ent_reg_rate = total_entropy_regressions / total_samples
    if total_ent_reg_rate > args.max_window_entropy_regression_rate:
        fail(
            f"total_entropy_regression_rate={total_ent_reg_rate} > "
    ...
    for window in sorted(window_samples):
        ...
        window_entropy = window_weighted_entropy[window] / samples
        if window_entropy < args.min_window_average_entropy_score:
            fail(
                f"window={window} average_entropy_score={window_entropy} < "
        ...
        if window_stability < args.min_window_average_stability_score:
            fail(
                f"window={window} average_stability_score={window_stability} < "
