...
def fail(message: str) -> None:
    print(f"E209 board threshold entropy budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument(
        "--threshold-entropy-budget-spent-field",
        default="threshold_entropy_budget_spent",
    )
    parser.add_argument("--max-threshold-entropy-budget-spent", type=float, default=1.0)
    parser.add_argument(
    ...
    for index, record in enumerate(records):
        threshold_entropy_budget_spent = to_float(
            record.get(args.threshold_entropy_budget_spent_field),
            args.threshold_entropy_budget_spent_field,
            index,
        )
        if threshold_entropy_budget_spent > args.max_threshold_entropy_budget_spent:
            fail(
                f"{args.threshold_entropy_budget_spent_field}="
                f"{threshold_entropy_budget_spent} > "
                f"{args.max_threshold_entropy_budget_spent} at index {index}"
            )

        over_threshold_entropy_budget_count = to_int(
            record.get(args.over_threshold_entropy_budget_count_field),
        ...
        if over_threshold_entropy_budget_count > args.max_over_threshold_entropy_budget_count:
            fail(
                ...
                f"{args.max_over_threshold_entropy_budget_count} at index {index}"
            )
