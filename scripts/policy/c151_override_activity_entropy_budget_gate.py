...
def fail(message: str) -> None:
    print(
        f"E151 C151 override activity entropy budget gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    rows = load_rows(args.overrides, ("overrides", "activity", "entropy"))
    total = 0
    breaches = 0
    for row in rows:
        ...
        budget = to_float(
            row.get(
                "override_activity_entropy_budget",
                row.get(
                    "activity_entropy_budget",
        ...
        actual = to_float(
            row.get(
                "override_activity_entropy_spend",
                row.get(
                    "activity_entropy_spend",
                    row.get("entropy_spend", row.get("cost", budget)),
                ),
            ),
            "override_activity_entropy_spend",
        )
        explicit_breach = to_bool(
            row.get(
                "budget_breach",
                row.get(
                    "override_activity_entropy_budget_breach",
                    row.get("activity_entropy_budget_breach", False),
                ),
            )
        )
