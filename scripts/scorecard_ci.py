...
PILLARS = [
    ...
    {"id":12,"name":"MAKEFILE","check":lambda p:any((p/n).exists() for n in ("Makefile", "Justfile", "justfile", "Taskfile.yml"))},
    ...
    {"id":28,"name":"INTEGRATION_TESTS","check":lambda p:(p/"tests"/"integration").exists() or (p/"test"/"integration").exists() or any(p.glob("tests/*_test.rs"))},
    ...
    {"id":33,"name":"SECURITY_SCANNING","check":lambda p:(p/".github/workflows"/"codeql.yml").exists() or (p/".github/workflows"/"trivy.yml").exists() or (p/".github/workflows"/"security.yml").exists() or (p/".gitleaks.toml").exists()},
    {"id":34,"name":"DEPENDENCY_AUDIT","check":lambda p:(p/".snyk").exists() or (p/"audit-ci.json").exists() or (p/"deny.toml").exists() or (p/".github/workflows"/"deny.yml").exists()},
    ...
    {"id":43,"name":"MONITORING","check":lambda p:(p/"monitoring").exists() or (p/"prometheus.yml").exists() or any(p.glob("**/prometheus*.yml")) or any(p.glob("**/prometheus*.yaml"))},
    ...
    {"id":54,"name":"ENV_VARS","check":lambda p:(p/".env").exists() or (p/".env.local").exists() or (p/".env.example").exists() or (p/".env.template").exists()},
    ...
    {"id":64,"name":"LICENSE_SCANNING","check":lambda p:(p/"license-checker.json").exists() or (p/"LICENSE_ALLOWLIST.toml").exists() or (p/".github/workflows"/"license-check.yml").exists()},
    ...
    {"id":81,"name":"INCIDENT_RESPONSE","check":lambda p:any(p.glob("**/*incident*response*")) or any(p.glob(".github/workflows/*incident*")) or any(p.glob(".github/ISSUE_TEMPLATE/*incident*"))},
    ...
    {"id":87,"name":"SHIPPING","check":lambda p:(p/".releaserc").exists() or (p/"release.config.js").exists() or any(p.glob(".github/workflows/*release*.yml")) or any(p.glob(".github/workflows/*release*.yaml"))},
