...
sys.path.insert(
    0,
    os.path.abspath(
        "/Users/kooshapari/Downloads/home-2/ubuntu/swe_agent_project/new/new"
    ),
)
from src.mcp.client import mcp_client_manager
...
async def main():
    ...
    tool = next(
        t for t in mcp_client_manager.tools if t.name == "canvas_list_student_courses"
    )
    ...
    if hasattr(tool, "coroutine"):
        ...
        print(
            f'Coroutine signature: {tool.coroutine.__code__.co_varnames if hasattr(tool.coroutine, "__code__") else "No code attribute"}'
        )
