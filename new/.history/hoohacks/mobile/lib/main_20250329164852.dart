    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (patterns.isEmpty)
            Card(
              // Wrap the message in a card for consistency
              child: Padding(
                padding: const EdgeInsets.all(16.0),
                child: Center(
                  child: Text(
  ...
  List<FlSpot> _getSpotsForMetric(String metricKey) {
    ...
    for (int i = 0; i < sortedLogs.length; i++) {
      ...
      if (metricKey == 'mood' || metricKey == 'energy') {
        ...
        // Check if healthData and the key exist
  ...
  Widget _buildChartCard({
    ...
    final finalMinY =
        calculatedMinY > calculatedMaxY
            ? calculatedMaxY - 1
            : calculatedMinY; // Ensure min < max
    final finalMaxY =
        calculatedMaxY < finalMinY
            ? finalMinY + 1
            : calculatedMaxY; // Ensure max > min
