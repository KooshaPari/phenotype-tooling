...
class AuthService {
  ...
  void _initializeAuth0() {
    try {
      ...
      if (domain == null || domain.isEmpty || clientId == null || clientId.isEmpty) {
  ...
  Future<void> init() async {
    if (!_isInitialized) {
      debugPrint('AuthService Warning: Attempting to use init() when not properly initialized');
      return;
    }
  ...
  Future<UserProfile?> login() async {
    if (!_isInitialized) {
      debugPrint('AuthService Warning: Attempting to use login() when not properly initialized');
      return null;
    }
    ...
    try {
      ...
      if (scheme == null || scheme.isEmpty) {
        debugPrint('AuthService Error: AUTH0_CUSTOM_SCHEME is missing or invalid in .env file');
        return null;
      }
  ...
  Future<void> logout() async {
    if (!_isInitialized) {
      debugPrint('AuthService Warning: Attempting to use logout() when not properly initialized');
      return;
    }
