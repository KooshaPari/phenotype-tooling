...
def convert_to_orchestration_request(openai_request: ChatCompletionRequest) -> Dict[str, Any]:
    """
...
def convert_to_openai_response(orchestration_response: Dict[str, Any], request: ChatCompletionRequest) -> ChatCompletionResponse:
    """
    ...
    response = ChatCompletionResponse(
        ...
        usage=ChatCompletionResponseUsage(
            prompt_tokens=len(" ".join([m.content for m in request.messages])) // 4,  # Rough estimate
            completion_tokens=len(text) // 4,  # Rough estimate
            total_tokens=(len(" ".join([m.content for m in request.messages])) + len(text)) // 4  # Rough estimate
        )
    )
