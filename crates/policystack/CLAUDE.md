...
## Policy-Contract Conditional Predicates (Authoritative Snippets)

For `policy-contract`, multi-condition Git predicates should use explicit `all` / `any` blocks:

```yaml
policy:
  command_rules:
    - id: policy_checkout_guard
      action: allow
      match: "git checkout*"
      on_mismatch: request
      conditions:
...
policy:
  command_rules:
    - id: policy_push_optional_checks
      action: allow
      match: "git push*"
      on_mismatch: request
      conditions:
...
Keep these snippets as the canonical shape for nested required/optional groups so nested
`any` / `all` decisions stay stable across evaluators.
