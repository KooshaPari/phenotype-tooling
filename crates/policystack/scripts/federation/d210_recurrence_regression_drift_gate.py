...
def fail(message: str) -> None:
    print(f"E210 recurrence regression budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)


def _require_file(path: pathlib.Path, label: str) -> None:
    if not path.is_file():
        fail(f"E210 missing {label} file: {path}")
...
def _to_records(payload: object, path: pathlib.Path, label: str) -> list[dict]:
    ...
    if isinstance(payload, list) and all(isinstance(item, dict) for item in payload):
        return payload
    fail(f"E210 invalid {label} JSON shape (expected object or list[object]): {path}")
    return []
...
def _read_json(path: pathlib.Path, label: str) -> list[dict]:
    try:
        ...
        fail(f"E210 invalid {label} JSON {path}: {exc}")
...
def _read_csv(path: pathlib.Path, required: set[str], label: str) -> list[dict[str, str]]:
    try:
        with path.open(newline="") as handle:
            ...
            if missing:
                fail(f"E210 {label} CSV missing headers {missing}: {path}")
            return list(reader)
    except SystemExit:
        raise
    except Exception as exc:
        fail(f"E210 invalid {label} CSV {path}: {exc}")
...
def _load_records(path: pathlib.Path, required: set[str], label: str) -> list[dict]:
    ...
    if suffix == ".json":
        ...
        for idx, row in enumerate(rows):
            ...
            if missing:
                fail(f"E210 {label} JSON row {idx} missing keys {missing}: {path}")
        return rows
    fail(f"E210 unsupported {label} format (expected .csv or .json): {path}")
    return []
...
def _to_float(value: object, path: pathlib.Path, field: str) -> float:
    try:
        ...
        fail(f"E210 invalid {field} in {path}: {exc}")
...
def main() -> int:
    ...
    if not rows:
        fail("E210 empty recurrence data")
    ...
    if regression_budget_gap > args.max_regression_budget_gap:
        fail(
            f"E210 regression_budget_gap={regression_budget_gap} > "
