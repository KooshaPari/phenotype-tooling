...
def fail(message: str) -> None:
    print(f"E181 override pressure stability budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)


def _require_file(path: pathlib.Path, label: str) -> None:
    if not path.is_file():
        fail(f"E181 missing {label} file: {path}")
...
def _to_records(payload: object, path: pathlib.Path, label: str) -> list[dict]:
    ...
    if isinstance(payload, list) and all(isinstance(item, dict) for item in payload):
        return payload
    fail(f"E181 invalid {label} JSON shape (expected object or list[object]): {path}")
    return []
...
def _read_json(path: pathlib.Path, label: str) -> list[dict]:
    try:
        ...
        fail(f"E181 invalid {label} JSON {path}: {exc}")
...
def _read_csv(path: pathlib.Path, required: set[str], label: str) -> list[dict[str, str]]:
    try:
        with path.open(newline="") as handle:
            ...
            if missing:
                fail(f"E181 {label} CSV missing headers {missing}: {path}")
            return list(reader)
    except SystemExit:
        raise
    except Exception as exc:
        fail(f"E181 invalid {label} CSV {path}: {exc}")
...
def _load_records(path: pathlib.Path, required: set[str], label: str) -> list[dict]:
    ...
    if suffix == ".json":
        ...
        for idx, row in enumerate(rows):
            ...
            if missing:
                fail(f"E181 {label} JSON row {idx} missing keys {missing}: {path}")
        return rows
    fail(f"E181 unsupported {label} format (expected .csv or .json): {path}")
    return []
...
def _to_float(value: object, path: pathlib.Path, field: str) -> float:
    try:
        ...
        fail(f"E181 invalid {field} in {path}: {exc}")
...
def main() -> int:
    ...
    parser.add_argument("--max-stability-gap", type=float, default=0.0)
    parser.add_argument("--max-stability-gap-mean", type=float, default=0.0)
    ...
    if len(rows) < 2:
        fail("E181 insufficient override history for stability calculations")
    ...
    paired_budgets = budgets[1:] if len(budgets) > 1 else []
    stability_gaps = [
    ...
    report_stability_gap = _to_float(
        report.get("override_pressure_stability_budget_gap_max", 0.0),
        report_path,
        "override_pressure_stability_budget_gap_max",
    )
    report_stability_gap_mean = _to_float(
        report.get("override_pressure_stability_budget_gap_mean", 0.0),
        report_path,
        "override_pressure_stability_budget_gap_mean",
    )
    report_over_budget_count = int(
        round(
            _to_float(
                report.get("override_pressure_stability_over_budget_count", 0),
                report_path,
                "override_pressure_stability_over_budget_count",
            )
        )
    )

    if stability_gap < report_stability_gap:
        stability_gap = report_stability_gap
    if stability_gap_mean < report_stability_gap_mean:
        stability_gap_mean = report_stability_gap_mean
    ...
    if stability_gap > args.max_stability_gap:
        fail(
            f"E181 stability_gap={stability_gap} > "
            f"max_stability_gap={args.max_stability_gap}"
        )
    if stability_gap_mean > args.max_stability_gap_mean:
        fail(
            f"E181 stability_gap_mean={stability_gap_mean} > "
            f"max_stability_gap_mean={args.max_stability_gap_mean}"
        )
    if over_budget_count > args.max_over_budget_count:
        fail(
            f"E181 over_budget_count={over_budget_count} > "
