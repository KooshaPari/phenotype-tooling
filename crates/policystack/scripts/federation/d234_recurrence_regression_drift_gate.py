...
def fail(message: str) -> None:
    print(f"E234 recurrence regression budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    if not rows:
        fail("E234 empty recurrence data")
    ...
    if regression_budget_gap > args.max_regression_budget_gap:
        fail(
            f"E234 regression_budget_gap={regression_budget_gap} > "
