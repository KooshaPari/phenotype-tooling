...
def parse_float(value: object, label: str) -> float:
    try:
        ...
        print(f"E130 invalid float for {label}: {value!r}", file=sys.stderr)
        raise SystemExit(2)
...
def parse_int(value: object, label: str) -> int:
    try:
        ...
        print(f"E130 invalid integer for {label}: {value!r}", file=sys.stderr)
        raise SystemExit(2)


parser = argparse.ArgumentParser()
parser.add_argument('--rollforward-window-report', required=True)
parser.add_argument('--max-rollforward-window-seconds', type=float, default=300.0)
parser.add_argument('--min-rollforward-window-success-rate', type=float, default=0.98)
parser.add_argument('--max-rollforward-window-breach-count', type=int, default=0)
args = parser.parse_args()

payload = load_report(pathlib.Path(args.rollforward_window_report))
rows = extract_rows(payload, 'schema_rollforward_window')

max_rollforward_window_seconds = 0.0
rollforward_window_success_rate = 1.0
...
for row in rows:
    ...
    rollforward_window_success_rate = min(
        rollforward_window_success_rate,
        parse_float(
            row.get(
                'rollforward_window_success_rate',
                row.get('schema_rollforward_success_rate', row.get('window_success_rate', 1.0)),
            ),
            'rollforward_window_success_rate',
        ),
    )
...
if (
    ...
    or rollforward_window_success_rate < args.min_rollforward_window_success_rate
    or rollforward_window_breach_count > args.max_rollforward_window_breach_count
):
    print('E130 schema rollforward window gate failed', file=sys.stderr)
    raise SystemExit(2)
