...
def fail(message: str) -> None:
    print(
        f"E175 C175 override activity window gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument("--max-window-activity", type=float, default=0.0)
    parser.add_argument("--max-window-activity-breaches", type=int, default=0)
    parser.add_argument("--max-window-activity-breach-rate", type=float, default=0.0)
    args = parser.parse_args()

    rows = load_rows(args.overrides, ("overrides", "activity", "window"))
    total = 0
    breaches = 0
    for row in rows:
        ...
        explicit_breach = to_bool(
            row.get(
                "activity_window_breach",
                row.get(
                    "window_breach",
                    row.get("activity_breach", False),
                ),
            )
        )
        if explicit_breach or (window_activity > args.max_window_activity):
            breaches += 1
