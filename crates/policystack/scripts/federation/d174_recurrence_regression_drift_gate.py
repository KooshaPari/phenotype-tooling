...
def fail(message: str) -> None:
    print(f"E174 recurrence regression budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)


def _require_file(path: pathlib.Path, label: str) -> None:
    if not path.is_file():
        fail(f"E174 missing {label} file: {path}")
...
def _to_records(payload: object, path: pathlib.Path, label: str) -> list[dict]:
    ...
    if isinstance(payload, list) and all(isinstance(item, dict) for item in payload):
        return payload
    fail(f"E174 invalid {label} JSON shape (expected object or list[object]): {path}")
    return []
...
def _read_json(path: pathlib.Path, label: str) -> list[dict]:
    try:
        ...
        fail(f"E174 invalid {label} JSON {path}: {exc}")
...
def _read_csv(path: pathlib.Path, required: set[str], label: str) -> list[dict[str, str]]:
    try:
        with path.open(newline="") as handle:
            ...
            if missing:
                fail(f"E174 {label} CSV missing headers {missing}: {path}")
            return list(reader)
    except SystemExit:
        raise
    except Exception as exc:
        fail(f"E174 invalid {label} CSV {path}: {exc}")
...
def _load_records(path: pathlib.Path, required: set[str], label: str) -> list[dict]:
    ...
    if suffix == ".json":
        ...
        for idx, row in enumerate(rows):
            ...
            if missing:
                fail(f"E174 {label} JSON row {idx} missing keys {missing}: {path}")
        return rows
    fail(f"E174 unsupported {label} format (expected .csv or .json): {path}")
    return []
...
def _to_float(value: object, path: pathlib.Path, field: str) -> float:
    try:
        ...
        fail(f"E174 invalid {field} in {path}: {exc}")
...
def main() -> int:
    ...
    if not rows:
        fail("E174 empty recurrence data")
    ...
    if regression_budget_gap > args.max_regression_budget_gap:
        fail(
            f"E174 regression_budget_gap={regression_budget_gap} > "
