...
def main() -> int:
    ...
    rows = extract_rows(payload, "schema_rollback_stability_budget")

    max_schema_rollback_seconds = 0.0
    schema_rollback_stability_rate = 1.0
    schema_rollback_stability_breach_count = 0

    for row in rows:
        max_schema_rollback_seconds = max(
            max_schema_rollback_seconds,
            parse_float(
                row.get(
                    "schema_rollback_seconds",
                    row.get("rollback_seconds", row.get("window_seconds", 0.0)),
                ),
                "schema_rollback_seconds",
            ),
        )
        schema_rollback_stability_rate = min(
            schema_rollback_stability_rate,
            parse_float(
                row.get(
                    "schema_rollback_stability_rate",
                    row.get(
                        "rollback_stability_rate",
        ...
        schema_rollback_stability_breach_count += parse_int(
            row.get(
                "schema_rollback_stability_breach_count",
                row.get("rollback_stability_breach_count", 0),
            ),
            "schema_rollback_stability_breach_count",
        )

    if (
        max_schema_rollback_seconds > args.max_schema_rollback_seconds
        or schema_rollback_stability_rate < args.min_schema_rollback_stability_rate
        or schema_rollback_stability_breach_count
        > args.max_schema_rollback_stability_breach_count
    ):
