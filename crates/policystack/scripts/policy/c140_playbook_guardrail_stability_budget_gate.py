...
def fail(message: str) -> None:
    print(
        f"E140 C140 playbook guardrail stability budget gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument("--max-instability-breaches", type=int, default=0)
    parser.add_argument("--max-instability-breach-rate", type=float, default=0.0)
    ...
    for row in rows:
        ...
        stability = to_float(
            row.get(
                "guardrail_stability",
                row.get("playbook_guardrail_stability", row.get("stability", 1.0)),
            ),
            "guardrail_stability",
        )
        ...
        budget = to_float(
            row.get(
                "guardrail_budget",
                row.get("playbook_guardrail_budget", row.get("budget", 0.0)),
            ),
            "guardrail_budget",
        )
        actual = to_float(
            row.get(
                "guardrail_cost",
                row.get("playbook_guardrail_cost", row.get("cost", budget)),
            ),
            "guardrail_cost",
        )
        explicit_breach = to_bool(
            row.get(
                "instability_breach",
                row.get("guardrail_budget_breach", row.get("budget_breach", False)),
            )
        )
    ...
    if args.max_instability_breaches and breaches > args.max_instability_breaches:
        fail(f"instability_breaches={breaches}")
    if args.max_instability_breach_rate and breach_rate > to_float(
        args.max_instability_breach_rate, "max_instability_breach_rate"
    ):
        fail(f"instability_breach_rate={breach_rate:.6f}")
    return 0
