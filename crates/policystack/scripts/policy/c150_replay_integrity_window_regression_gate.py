...
def fail(message: str) -> None:
    print(
        f"E150 C150 replay integrity window regression gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    for row in rows:
        ...
        budget = to_float(
            row.get(
                "replay_integrity_budget",
                row.get("integrity_budget", row.get("budget", 0.0)),
            ),
            "replay_integrity_budget",
        )
        actual = to_float(
            row.get(
                "replay_integrity_cost",
                row.get("integrity_cost", row.get("cost", budget)),
            ),
            "replay_integrity_cost",
        )
        regression = to_bool(
            row.get(
                "regression",
                row.get("replay_integrity_regression", row.get("integrity_regression", False)),
            )
        ...
        if regression or budget_breach or (actual - budget > args.allowed_budget_overrun):
            breaches += 1
