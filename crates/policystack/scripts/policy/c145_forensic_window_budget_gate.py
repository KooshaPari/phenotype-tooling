...
def fail(message: str) -> None:
    print(
        f"E145 C145 forensic window budget gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    rows = load_rows(args.forensics, ("forensics", "windows", "window"))
    total = 0
    breaches = 0
    for row in rows:
        ...
        budget = to_float(
            row.get(
                ...
                row.get("forensic_budget", row.get("budget", 0.0)),
            ),
            "forensic_window_budget",
        )
        actual = to_float(
            row.get(
                ...
                row.get("forensic_cost", row.get("cost", budget)),
            ),
            "forensic_window_cost",
        )
