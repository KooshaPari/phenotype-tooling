...
def main() -> int:
    ...
    for index, record in enumerate(records):
        threshold_stability_budget_spent = to_float(
            record.get(args.threshold_stability_budget_spent_field),
            args.threshold_stability_budget_spent_field,
            index,
        )
        if threshold_stability_budget_spent > args.max_threshold_stability_budget_spent:
            fail(
                f"{args.threshold_stability_budget_spent_field}={threshold_stability_budget_spent} > "
                f"{args.max_threshold_stability_budget_spent} at index {index}"
            )

        over_threshold_stability_budget_count = to_int(
            record.get(args.over_threshold_stability_budget_count_field),
        ...
        if over_threshold_stability_budget_count > args.max_over_threshold_stability_budget_count:
            fail(
                ...
                f"{args.max_over_threshold_stability_budget_count} at index {index}"
            )
