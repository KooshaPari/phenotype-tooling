...
def main() -> int:
    ...
    for index, record in enumerate(records):
        exception_regression_budget_spent = to_float(
            record.get(args.exception_regression_budget_spent_field),
        ...
        if exception_regression_budget_spent > args.max_exception_regression_budget_spent:
            fail(
                ...
                f"{args.max_exception_regression_budget_spent} at index {index}"
            )

        over_exception_regression_budget_count = to_int(
            record.get(args.over_exception_regression_budget_count_field),
            args.over_exception_regression_budget_count_field,
            index,
        )
        if (
            over_exception_regression_budget_count
            > args.max_over_exception_regression_budget_count
        ):
            fail(
                f"{args.over_exception_regression_budget_count_field}="
                f"{over_exception_regression_budget_count} > "
                f"{args.max_over_exception_regression_budget_count} at index {index}"
            )
