...
def main() -> int:
    ...
    for index, record in enumerate(records):
        exception_window_budget_spent = to_float(
            record.get(args.exception_window_budget_spent_field),
            args.exception_window_budget_spent_field,
            index,
        )
        if exception_window_budget_spent > args.max_exception_window_budget_spent:
            fail(
                f"{args.exception_window_budget_spent_field}={exception_window_budget_spent} > "
                f"{args.max_exception_window_budget_spent} at index {index}"
            )

        over_exception_window_budget_count = to_int(
            record.get(args.over_exception_window_budget_count_field),
