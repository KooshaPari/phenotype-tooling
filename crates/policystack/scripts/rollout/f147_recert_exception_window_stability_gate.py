...
def main() -> int:
    ...
    for index, record in enumerate(records):
        exception_window_stability_score = to_float(
            record.get(args.exception_window_stability_score_field),
            args.exception_window_stability_score_field,
            index,
        )
        if exception_window_stability_score < args.min_exception_window_stability_score:
            fail(
                f"{args.exception_window_stability_score_field}={exception_window_stability_score} < "
                f"{args.min_exception_window_stability_score} at index {index}"
            )

        unstable_exception_window_count = to_int(
            record.get(args.unstable_exception_window_count_field),
            args.unstable_exception_window_count_field,
            index,
        )
        if unstable_exception_window_count > args.max_unstable_exception_window_count:
            fail(
                f"{args.unstable_exception_window_count_field}={unstable_exception_window_count} > "
                f"{args.max_unstable_exception_window_count} at index {index}"
            )
