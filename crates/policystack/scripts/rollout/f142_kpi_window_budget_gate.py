...
def fail(message: str) -> None:
    print(f"E142 kpi window budget gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def main() -> int:
    ...
    for index, record in enumerate(records):
        window_budget_spent = to_float(
            record.get(args.window_budget_spent_field),
            args.window_budget_spent_field,
            index,
        )
        if window_budget_spent > args.max_window_budget_spent:
            fail(
                f"{args.window_budget_spent_field}={window_budget_spent} > "
                f"{args.max_window_budget_spent} at index {index}"
            )

        over_window_budget_count = to_int(
            record.get(args.over_window_budget_count_field),
            args.over_window_budget_count_field,
            index,
        )
        if over_window_budget_count > args.max_over_window_budget_count:
            fail(
                f"{args.over_window_budget_count_field}={over_window_budget_count} > "
                f"{args.max_over_window_budget_count} at index {index}"
            )
