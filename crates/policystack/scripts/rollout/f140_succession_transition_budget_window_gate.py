...
def main() -> int:
    ...
    for index, record in enumerate(records):
        transition_budget_window_spent = to_float(
            record.get(args.transition_budget_window_spent_field),
            args.transition_budget_window_spent_field,
            index,
        )
        if transition_budget_window_spent > args.max_transition_budget_window_spent:
            fail(
                f"{args.transition_budget_window_spent_field}={transition_budget_window_spent} > "
                f"{args.max_transition_budget_window_spent} at index {index}"
            )

        over_transition_budget_window_count = to_int(
            record.get(args.over_transition_budget_window_count_field),
        ...
        if over_transition_budget_window_count > args.max_over_transition_budget_window_count:
            fail(
                ...
                f"{args.max_over_transition_budget_window_count} at index {index}"
            )
