...
def main() -> int:
    ...
    for index, record in enumerate(records):
        transition_entropy_budget_spent = to_float(
            record.get(args.transition_entropy_budget_spent_field),
            args.transition_entropy_budget_spent_field,
            index,
        )
        if transition_entropy_budget_spent > args.max_transition_entropy_budget_spent:
            fail(
                f"{args.transition_entropy_budget_spent_field}={transition_entropy_budget_spent} > "
                f"{args.max_transition_entropy_budget_spent} at index {index}"
            )

        over_transition_entropy_budget_count = to_int(
            record.get(args.over_transition_entropy_budget_count_field),
        ...
        if over_transition_entropy_budget_count > args.max_over_transition_entropy_budget_count:
            fail(
                ...
                f"{args.max_over_transition_entropy_budget_count} at index {index}"
            )
