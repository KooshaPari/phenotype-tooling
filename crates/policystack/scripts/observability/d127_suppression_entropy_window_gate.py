...
def fail(message: str) -> None:
    print(f"E127 suppression entropy window gate failed: {message}", file=sys.stderr)
    raise SystemExit(2)


def _require_file(path: pathlib.Path, label: str) -> None:
    if not path.is_file():
        fail(f"E127 missing {label} file: {path}")
...
def _to_records(payload: object, path: pathlib.Path, label: str) -> list[dict]:
    ...
    if isinstance(payload, list) and all(isinstance(item, dict) for item in payload):
        return payload
    fail(f"E127 invalid {label} JSON shape (expected object or list[object]): {path}")
    return []
...
def _read_json(path: pathlib.Path, label: str) -> list[dict]:
    try:
        ...
        fail(f"E127 invalid {label} JSON {path}: {exc}")
...
def _read_csv(path: pathlib.Path, required: set[str], label: str) -> list[dict[str, str]]:
    try:
        with path.open(newline="") as handle:
            ...
            if missing:
                fail(f"E127 {label} CSV missing headers {missing}: {path}")
            return list(reader)
    except SystemExit:
        raise
    except Exception as exc:
        fail(f"E127 invalid {label} CSV {path}: {exc}")
...
def _load_records(path: pathlib.Path, required: set[str], label: str) -> list[dict]:
    ...
    if suffix == ".json":
        ...
        for idx, row in enumerate(rows):
            ...
            if missing:
                fail(f"E127 {label} JSON row {idx} missing keys {missing}: {path}")
        return rows
    fail(f"E127 unsupported {label} format (expected .csv or .json): {path}")
    return []
...
def _to_float(value: object, path: pathlib.Path, field: str) -> float:
    try:
        ...
        fail(f"E127 invalid {field} in {path}: {exc}")
...
def main() -> int:
    ...
    parser.add_argument("--max-window-rise", type=float, default=0.0)
    ...
    if not rows:
        fail("E127 empty suppression data")
    ...
    window_span = _window_max_span(entropies, args.window_size)
    window_rise = _window_max_rise(entropies, args.window_size)

    report_span = _to_float(
        report.get("suppression_entropy_window_span_max", 0.0),
        report_path,
        "suppression_entropy_window_span_max",
    )
    report_rise = _to_float(
        report.get("suppression_entropy_window_rise_max", 0.0),
        report_path,
        "suppression_entropy_window_rise_max",
    )

    if window_span < report_span:
        window_span = report_span
    if window_rise < report_rise:
        window_rise = report_rise

    if window_span > args.max_window_span:
        fail(f"E127 window_span={window_span} > max_window_span={args.max_window_span}")
    if window_rise > args.max_window_rise:
        fail(f"E127 window_rise={window_rise} > max_window_rise={args.max_window_rise}")
