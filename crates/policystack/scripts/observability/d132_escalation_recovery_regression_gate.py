...
def _require_file(path: pathlib.Path, label: str) -> None:
    if not path.is_file():
        fail(f"E132 missing {label} file: {path}")
...
def _to_records(payload: object, path: pathlib.Path, label: str) -> list[dict]:
    ...
    if isinstance(payload, list) and all(isinstance(item, dict) for item in payload):
        return payload
    fail(f"E132 invalid {label} JSON shape (expected object or list[object]): {path}")
    return []
...
def _read_json(path: pathlib.Path, label: str) -> list[dict]:
    try:
        ...
        fail(f"E132 invalid {label} JSON {path}: {exc}")
...
def _read_csv(path: pathlib.Path, required: set[str], label: str) -> list[dict[str, str]]:
    try:
        with path.open(newline="") as handle:
            ...
            if missing:
                fail(f"E132 {label} CSV missing headers {missing}: {path}")
            return list(reader)
    except SystemExit:
        raise
    except Exception as exc:
        fail(f"E132 invalid {label} CSV {path}: {exc}")
...
def _load_records(path: pathlib.Path, required: set[str], label: str) -> list[dict]:
    ...
    if suffix == ".json":
        ...
        for idx, row in enumerate(rows):
            ...
            if missing:
                fail(f"E132 {label} JSON row {idx} missing keys {missing}: {path}")
        return rows
    fail(f"E132 unsupported {label} format (expected .csv or .json): {path}")
    return []
...
def _to_float(value: object, path: pathlib.Path, field: str) -> float:
    try:
        ...
        fail(f"E132 invalid {field} in {path}: {exc}")
...
def main() -> int:
    ...
    parser.add_argument("--max-recovery-lag-regression", type=float, default=0.0)
    ...
    recovery_lag_regression = _max_positive_step(recovery_lags)
    regression_score_max = max(regressions) if regressions else 0.0
    ...
    report_lag_regression = _to_float(
        report.get("escalation_recovery_lag_regression_max", 0.0),
        report_path,
        "escalation_recovery_lag_regression_max",
    )
    report_regression_score = _to_float(
        report.get("escalation_regression_score_max", 0.0),
        report_path,
        "escalation_regression_score_max",
    )

    if recovery_ratio_gap < report_ratio_gap:
        recovery_ratio_gap = report_ratio_gap
    if recovery_lag_regression < report_lag_regression:
        recovery_lag_regression = report_lag_regression
    if regression_score_max < report_regression_score:
        regression_score_max = report_regression_score

    if recovery_ratio_gap > args.max_recovery_ratio_gap:
        fail(
            f"E132 recovery_ratio_gap={recovery_ratio_gap} > "
            f"max_recovery_ratio_gap={args.max_recovery_ratio_gap}"
        )
    if recovery_lag_regression > args.max_recovery_lag_regression:
        fail(
            f"E132 recovery_lag_regression={recovery_lag_regression} > "
            f"max_recovery_lag_regression={args.max_recovery_lag_regression}"
        )
    if regression_score_max > args.max_regression_score:
        fail(
            f"E132 regression_score_max={regression_score_max} > "
