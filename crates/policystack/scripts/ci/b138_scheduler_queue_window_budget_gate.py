...
def fail(message: str) -> None:
    print(
        f"E138 [lane B] scheduler queue window budget gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument("--max-total-queue-overflow-events", type=int, default=0)
    ...
    for row in records:
        ...
        if queue_stall_events < 0:
            fail(
                f"queue_stall_events for {args.queue_stall_events_key} must be >= 0; got "
                f"{queue_stall_events}"
            )
    ...
    if total_queue_overflow_events > args.max_total_queue_overflow_events:
        fail(
            f"total_queue_overflow_events={total_queue_overflow_events} > "
            f"max_total_queue_overflow_events={args.max_total_queue_overflow_events}"
        )
    ...
    for window in sorted(window_queue_budget_used):
