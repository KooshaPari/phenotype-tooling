...
def fail(message: str) -> None:
    print(
        f"E129 [lane B] signature preflight window regression gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    for row in records:
        ...
        if critical_regressions > regressions:
            fail(
                f"critical regressions={critical_regressions} cannot exceed regressions={regressions}"
            )
    ...
    for window in sorted(window_samples):
        ...
        rate = window_regressions[window] / samples
        if rate > args.max_window_regression_rate:
            fail(
                f"window={window} regression_rate={rate} > "
