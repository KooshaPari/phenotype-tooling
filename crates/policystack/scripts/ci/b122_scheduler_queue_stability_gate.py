...
def fail(message: str) -> None:
    print(
        f"E122 [lane B] scheduler queue stability gate failed: {message}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    parser.add_argument("--stalled-flag-key", default="stalled")
    ...
    parser.add_argument("--max-average-queue-jitter", type=float, default=0.0)
    parser.add_argument("--max-stalled-count", type=int, default=0)
    ...
    queue_depth_total = 0.0
    queue_jitter_total = 0.0
    max_queue_depth = 0.0
    stalled_count = 0
    ...
    for row in records:
        ...
        queue_jitter_total += queue_jitter
        max_queue_depth = max(max_queue_depth, queue_depth)
        stalled_count += 1 if stalled else 0

        window = str(row.get(args.window_key, "default"))
        window_totals[window] = window_totals.get(window, 0.0) + queue_depth
        window_counts[window] = window_counts.get(window, 0) + 1

    average_queue_depth = queue_depth_total / len(records)
    if average_queue_depth > args.max_average_queue_depth:
        fail(
            f"average_queue_depth={average_queue_depth} > max_average_queue_depth="
            f"{args.max_average_queue_depth}"
        )
    ...
    if average_queue_jitter > args.max_average_queue_jitter:
        fail(
            f"average_queue_jitter={average_queue_jitter} > max_average_queue_jitter="
            f"{args.max_average_queue_jitter}"
        )
    ...
    for window, count in sorted(window_counts.items()):
        ...
        if average > args.max_window_average_queue_depth:
            fail(
                f"window={window} average_queue_depth={average} > max_window_average_queue_depth="
                f"{args.max_window_average_queue_depth}"
            )
