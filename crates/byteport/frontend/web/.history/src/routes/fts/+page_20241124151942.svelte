<script lang="ts">
	...
	function firstTimeSetup() {
		addtl = true;
		if (client) {
			userData = { ...userData, UUID: client.UUID, Name: client.Name, Email: client.Email };
		}
		setStage();
	...
	async function Link() {
		try {
            // Update the store with the complete userData
            setUser(true, userData);
            ...
            const response = await fetch(`${SERVER_URL}/link`, {
                ...
                credentials: 'include', // Ensure cookies are sent if using HTTP-only cookies
            ...
            if (response.ok) {
                ...
                // Optionally, update the store with any additional data from the server
            } else {
