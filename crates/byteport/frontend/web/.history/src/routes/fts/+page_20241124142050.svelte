<script lang="ts">
	...
	function setStage() {
		...
		if (currentStageForm && !currentStageForm.checkValidity()) {
			...
			// write down form items to user store a
			const formData = new FormData(currentStageForm);
			const data = Object.fromEntries(formData) as UserStore;
			// set fields in userData (just the fields these entries contain, dont overwrite the rest)
    ...
    function Link(){
        ...
        fetch(`${SERVER_URL}/link`, {
            ...
            headers: {
                ...
                'Authorization': `Bearer ${user.data.Encrypted_Token}`
            },
