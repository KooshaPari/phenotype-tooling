...
<div class="h-screen w-screen overflow-x-hidden bg-dark-surface">
	...
	<div id="body" class="px-2.5 pt-5">
		...
		<div id="signCont">
			<form class="flex-row" name="regUser" action="/fts" on:submit={signUpUser} method="post">
				<div>
					<label for="name">Name</label>
					<input name="name" type="text" pattern="" required placeholder="Username" />
				</div>
				<div>
				...
				<div>
					...
					<input name="password" 
                    pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"type="password" required placeholder="Password" />
				</div>
				<input
					type="submit"
					value="Sign Up"
