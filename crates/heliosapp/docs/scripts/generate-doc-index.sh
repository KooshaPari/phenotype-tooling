...
if command -v rg >/dev/null 2>&1; then
  ...
  all_files=$(find "$ROOT_DIR" -type f -name "*.md" \
    ! -path "*/node_modules/*" \
    ! -path "*/.worktrees/*" \
    ! -path "*/.generated/*" \
    ! -path "*/coverage/*" \
...
write_bucket() {
  local title="$1"; local out="$2"; local pattern="$3"
  { echo "# $title"; echo; echo "Autogenerated by docs/scripts/generate-doc-index.sh"; echo
    while IFS= read -r f; do rel="${f#${ROOT_DIR}/}"; if [[ "$rel" =~ $pattern ]]; then echo "- [$rel](../${rel})"; fi; done <<< "$all_files"
  } > "$out"
}
{ echo '# Raw/All'; echo; echo 'Autogenerated by docs/scripts/generate-doc-index.sh'; echo
  while IFS= read -r f; do rel="${f#${ROOT_DIR}/}"; echo "- [$rel](../${rel})"; done <<< "$all_files"
