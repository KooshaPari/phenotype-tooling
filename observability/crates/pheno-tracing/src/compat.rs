...
mod tests {
    ...
    #[test]
    fn shim_version_is_valid_semver() {
        // The shim is GA at 0.1.0 today; the `tracing-0-2` Cargo feature
        // is the forward-compat hook that will be flipped when upstream
        // `tracing 0.2` ships. Until then the shim is part of the stable
        // 0.1.0 release, not a pre-release. This test guards against
        // future drift: a non-empty, semver-shaped `SHIM_VERSION` is the
        // contract downstream code relies on for OTLP resource-attribute
        // encoding and the `tracing-0-2` feature flag decision.
        let v = SHIM_VERSION;
        assert!(!v.is_empty(), "SHIM_VERSION must not be empty");
        // Lightweight semver check: major.minor.patch, with an optional
        // pre-release / build suffix. The point is to catch obvious drift
        // (empty string, garbage) without pulling in a semver crate just
        // for a unit test.
        ...
        assert_eq!(
            parts.len(),
            3,
            "SHIM_VERSION must be major.minor.patch[ -suffix], got: {v}"
        );
        for part in &parts {
            assert!(
                !part.is_empty() && part.chars().all(|c| c.is_ascii_digit()),
                "SHIM_VERSION numeric parts must be non-empty digits, got: {v}"
            );
        }
    }
