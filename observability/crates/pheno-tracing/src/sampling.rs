...
///// re-baselined to MSRV 1.75 on 2026-09-08).
///
/// On poison, emit a `tracing::warn!` (tagged `pheno_tracing.sampling` for
/// fleet-level filtering) and recover the inner guard so the trace path
/// stays alive. This mirrors the recovery pattern already used by
/// `InMemoryAdapter::submit` in `adapters.rs`.
///
...
/// # MSRV note
///
/// `Mutex::clear_poison` was stabilized in Rust 1.77. The crate's MSRV is
/// 1.75, so this helper takes `PoisonError::into_inner()` instead of
/// calling `clear_poison()` + `lock()`. The semantic difference is benign:
/// the mutex stays "poisoned" from the standard library's perspective,
/// but the next call to this helper will again hit the `Err` branch and
/// recover via `into_inner()`. From the sampler's point of view every
/// call returns a valid `MutexGuard` so the trace path is never blocked
/// by a stale poison flag. When the crate's MSRV is bumped to 1.77, this
/// helper can be simplified to call `clear_poison()` + `lock()` and
/// subsequent calls will take the `Ok` branch directly.
#[inline]
fn lock_or_recover<'a, T>(mutex: &'a Mutex<T>, what: &'static str) -> MutexGuard<'a, T> {
    match mutex.lock() {
        Ok(g) => g,
        Err(poisoned) => {
            // Poison detected — recover the inner guard and log so operators
            // can spot a thread panic that left the sampler state in an
            // unknown shape. We use `PoisonError::into_inner()` because it
            // is stable on Rust 1.75; the equivalent `clear_poison()` call
            // only became stable in 1.77.
            tracing::warn!(
