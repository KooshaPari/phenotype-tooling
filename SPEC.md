# phenotype-tooling - Specification

## Problem

`phenotype-tooling` addresses a specific need in the Phenotype fleet: phenotype org internal tooling: usage-poll, agent-forecast, temporal-grounding. Without a canonical implementation, downstream consumers must reinvent the same primitives, leading to fragmentation and divergent behavior across the fleet.

## Solution

`phenotype-tooling` provides a single, well-tested, well-documented implementation of this capability. The package ships with:
- A stable public API
