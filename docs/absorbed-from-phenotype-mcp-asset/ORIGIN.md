...
This crate was **extracted** (not rewritten) from a parent repo that is now
archived. This document records the full chain of custody.
...
| Field | Value |
|---|---|
| **Destination repo** | `KooshaPari/phenotype-mcp-asset` |
| **Destination status** | Pending GitHub repo creation (see below) |
| **Destination version** | **v0.3.0** (bumped from 0.2.0 because this is a standalone lib, not a McpKit workspace member) |
...
**Repository live at:** <https://github.com/KooshaPari/phenotype-mcp-asset>
...
└── rust/
    └── phenotype-mcp-asset/         (source path, v0.2.0)
        ...
        └── tests/                    │ (empty in source)
                                     ─┘
                                     ↓
KooshaPari/phenotype-mcp-asset       (destination repo, v0.3.0, PENDING CREATE)
