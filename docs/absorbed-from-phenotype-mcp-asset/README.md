> **ARCHIVED** (2026-06-18) — Disposition index `disposition-index.json` row id=52: fsm=done, target=standalone repo. This crate had 9 placeholder modules, no MCP integration, no consumers, no CI, no LICENSE. Read-only. No further changes will be accepted.
...
```text
version: 0.3.0
tier:    pheno-*-lib (per ADR-023)
origin:  extracted from KooshaPari/McpKit v0.2.0 on 2026-06-18
```
...
```rust
use phenotype_mcp_asset::AssetHandler;
...
`phenotype-mcp-asset-sdk` (per ADR-042) requires 2+ polyglot consumers
...
- [`ORIGIN.md`](./ORIGIN.md) — full extraction provenance from `KooshaPari/McpKit` v0.2.0
