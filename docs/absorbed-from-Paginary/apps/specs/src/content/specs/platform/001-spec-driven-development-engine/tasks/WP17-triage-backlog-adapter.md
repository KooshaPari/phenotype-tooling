...
3. **Conventional commits**: All commits must follow the pattern ``<type>(<scope>): <description>``. Valid types are `feat`, `fix`, `chore`, `refactor`, `docs`, `test`, and `ci`. Scope is the WP ID (e.g., `WP17`).
