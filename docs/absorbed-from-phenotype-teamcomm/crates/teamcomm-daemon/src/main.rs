...
fn init_tracing() {
    // Priority: RUST_LOG env var > TEAMCOMM_LOG_LEVEL env var > "info"
    let filter = EnvFilter::try_from_default_env()
        .or_else(|_| {
            std::env::var("TEAMCOMM_LOG_LEVEL")
...
async fn cmd_stop(socket_path: Option<PathBuf>, pid_file: Option<PathBuf>) -> Result<()> {
    ...
    // Use the configured stop timeout (retry every 100ms).
    let stop_timeout = DaemonConfig::load().stop_timeout_sec;
    let retries = stop_timeout * 10; // 100ms per iteration
    for _ in 0..retries {
        if !teamcomm_daemon::pid::is_pid_running(pid) {
            teamcomm_daemon::pid::remove_pid_file(&pid_file)?;
    ...
    warn!(
        ...
        "daemon did not exit within {}s; removing pid file anyway",
        stop_timeout,
    );
    teamcomm_daemon::pid::remove_pid_file(&pid_file)?;
    Ok(())
