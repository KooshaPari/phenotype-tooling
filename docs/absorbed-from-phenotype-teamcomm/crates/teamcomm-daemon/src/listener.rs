...
//!
//! Limits (max line bytes, idle timeout) are read from
//! [`DaemonConfig`] so operators can tune them without a recompile.
...
// MAX_LINE_BYTES and IDLE_TIMEOUT have moved to [`DaemonConfig`].
            ...
            accept = listener.accept() => {
                match accept {
                    Ok((stream, _addr)) => {
                        ...
                        let conn_task = tokio::spawn(handle_connection(stream, state, max_line_bytes, idle_timeout));
...
mod tests {
    ...
    async fn run_starts_and_stops_on_shutdown_signal() {
        ...
        let idle_timeout = Duration::from_secs(cfg.idle_timeout_sec);
        ...
        assert!(resp_json["result"]["session_id"]
            ...
            .starts_with("sess_"));
        assert_eq!(
            resp_json["result"]["lease_ttl_sec"],
            cfg.heartbeat_timeout_sec
        );
