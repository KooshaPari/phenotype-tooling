...
1. **GitHub Security Advisories** (preferred) — open a private advisory at
   `https://github.com/Phenotype-org/phenotype-terrain/security/advisories/new`.
...
- The Unity version, WorldBox install path, and the affected public API surface
  (e.g. `HeightField`, `ChunkMeshBuilder`, `TerrainLod`)

## Scope

The following are in scope:

- Public API surface of `Phenotype.Terrain` that ships to consumers as a sibling
  project reference
...
- Source-level issues in this repository
...
mods. Most consumers do not run their own network stack on top of it; the
attack surface is dominated by malformed inputs to the public API
(`HeightField.Set`, `ChunkMeshBuilder.Build`, `TerrainLod.Select`). When in
