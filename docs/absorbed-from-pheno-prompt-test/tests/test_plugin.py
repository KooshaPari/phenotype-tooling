...
def test_compute_similarity_overlap():
    ...
    # {the, brown} ∩ {the, slow, brown, dog} = 2
    # {the, quick, brown, fox} ∪ {the, slow, brown, dog} = 6
    # 2/6 = 0.333
    assert 0.30 <= s <= 0.36
...
def test_find_prompt_cases(tmp_path):
    ...
    prompts_dir.mkdir()
    (prompts_dir / "case_a.prompt").write_text(
        "name: a\nprompt: |\n  hello\n"
    )
    (prompts_dir / "case_b.prompt").write_text(
        "name: b\nprompt: |\n  world\n"
    )
