...
Install with: ``pip install phenotype-py-extras[mcp]``
...
# ``fastmcp`` and ``pydantic_settings`` are imported under canonical names.
_LAZY_ATTRS = {
    "fastmcp": "fastmcp",
    "pydantic": "pydantic",
...
def __getattr__(name: str) -> Any:
    ...
    if target is None:
        raise AttributeError(f"module 'phenotype_py_extras.mcp' has no attribute {name!r}")
    import importlib
