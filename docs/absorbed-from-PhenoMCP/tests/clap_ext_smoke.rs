...
#[test]
fn clap_ext_config_arg_parses_long_flag() {
    #[derive(Parser)]
    struct Probe {
        #[command(flatten)]
        config: ConfigArg,
    }
    let p = Probe::try_parse_from(["probe", "--config", "/etc/phenomcp.toml"]).expect("parse");
    assert_eq!(p.config.config.unwrap().to_str().unwrap(), "/etc/phenomcp.toml");
}
