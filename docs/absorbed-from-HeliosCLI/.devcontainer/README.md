...
We provide the following options to facilitate Helios CLI development in a container. This is particularly useful for verifying the Linux build when working on a macOS host.
...
```shell
HELIOS_DOCKER_IMAGE_NAME=helios-cli-linux-dev
docker build --platform=linux/amd64 -t "$HELIOS_DOCKER_IMAGE_NAME" ./.devcontainer
docker run --platform=linux/amd64 --rm -it -e CARGO_TARGET_DIR=/workspace/helios-rs/target-amd64 -v "$PWD":/workspace -w /workspace/helios-rs "$HELIOS_DOCKER_IMAGE_NAME"
```
...
VS Code recognizes the `devcontainer.json` file and gives you the option to develop Helios CLI in a container. Currently, `devcontainer.json` builds and runs the `arm64` flavor of the container.
