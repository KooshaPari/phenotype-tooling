...
Usage in git hooks:
    python -c "from pheno.deployment.hooks import pre_push_check; exit(pre_push_check())"
"""
...
from pheno.deployment.checks import VendorChecker
...
def get_changed_files() -> list[str]:
    ...
    try:
        ...
        # Fallback to HEAD~1
        result = subprocess.run(
            ["git", "diff", "--name-only", "HEAD~1"], capture_output=True, text=True, check=False,
        )
    ...
    if requirements_changed(changed_files):
        ...
        if not vendor_dir.exists():
            if not quiet:
                ...
                print(
                    "[pre-push] Run 'pheno-vendor setup' to initialize vendoring", file=sys.stderr,
                )
