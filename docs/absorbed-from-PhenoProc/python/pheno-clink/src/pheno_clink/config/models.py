"""
Pydantic models for CLI client configuration.
"""
...
class OutputCaptureConfig(BaseModel):
    ...
    flag_template: str = Field(
        ..., description="Template used to inject the output path, e.g. '--output {path}'.",
    )
    cleanup: bool = Field(
        default=True,
...
class ResolvedCLIClient(BaseModel):
    ...
    def get_role(self, role_name: str | None) -> ResolvedCLIRole:
        ...
        if key not in self.roles:
            ...
            raise KeyError(
                f"Role '{role_name}' not configured for CLI '{self.name}'. Available roles: {available}",
            )
...
class ClientConfig(BaseModel):
    ...
    cli_clients: dict[str, CLIClientConfig] = Field(
        default_factory=dict, description="Map of client ID to configuration",
    )
    global_overrides: dict[str, Any] = Field(
        default_factory=dict,
        description="Global overrides applied to all clients (e.g., default timeout)",
    )
