...
## Configuration Files
## Configuration (via pheno-sdk)

Use pheno-sdk's config-kit as the single source of truth for configuration.

```python
from kinfra import PhenoConfig  # pheno-sdk's Config, re-exposed by kinfra

class AppConfig(PhenoConfig):
    debug: bool = False
    port: int = 8000

# Merge env vars > config file > defaults
config = AppConfig.load(env_prefix="APP_", config_file="config.yaml", defaults={"debug": False})
```

To layer .env files deterministically, use config-kit loaders:

```python
from config_kit.loaders.env_cascade import load_env_cascade
from pathlib import Path

env = load_env_cascade(root_dirs=[Path(".")], env_files=[Path(".env.local")])
```


## Migration Notes (Configuration)

- KInfra no longer exposes legacy config helpers. Use pheno-sdk config-kit exclusively.
- Replace any previous imports like `from lib.service_orchestration import load_env_cascade` with:

```python
from config_kit.loaders.env_cascade import load_env_cascade
```

- For typed app settings, use pheno-sdk Config (re-exposed by kinfra):

```python
from kinfra import PhenoConfig
class AppConfig(PhenoConfig):
    debug: bool = False
    port: int = 8000
```

- The canonical APIs listed below are authoritative; no legacy aliases remain.
...
- KInfra.create_tunnel(service, port, domain=None, path="/")
- KInfra.create_quick_tunnel(service, port, protocol="http")
- KInfra.stop_tunnel(service), KInfra.stop_all_tunnels()
- KInfra.get_public_url(service), KInfra.get_port(service), KInfra.get_info(service)
- KInfra.check_health(service)
- KInfra.ensure_port_and_tunnel(service, preferred_port=None, domain=None, path="/")
