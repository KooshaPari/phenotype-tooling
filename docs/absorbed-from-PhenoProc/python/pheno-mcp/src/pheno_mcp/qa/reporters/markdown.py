...
class MarkdownReporter:
    ...
    def report(self, results: list[dict[str, Any]], metadata: dict[str, Any]) -> None:
        ...
        # Auth/MFA structured events (redacted)
        has_events = any((r.get("events") or []) for r in results)
        if has_events:
            lines.append("\n## Auth/MFA Events (redacted)\n")
            for r in results:
                evs = r.get("events") or []
                if not evs:
                    continue
                lines.append(f"### {r.get('test_name', 'unknown')}")
                for ev in evs[-10:]:  # show last 10 events per test
                    typ = ev.get("type", "")
                    ts = ev.get("ts")
                    when = (
                    ...
                    summary_parts = []
                    for key in ("provider", "method", "adapter"):
                        if key in ev:
                            summary_parts.append(f"{key}={ev[key]}")
                    details = f" ({', '.join(summary_parts)})" if summary_parts else ""
                    lines.append(f"- {when} {typ}{details}")
