"""Cost calculator middleware for pheno-mcp-router.
...
returned by the upstream provider whenever possible.

Ported from dispatch-mcp W2-1 (commit ``6aad7fa``) per L5-104.1
as substrate-level cost calc per ADR-013. Module imports were
rewritten from ``dispatch_mcp.core.X`` to ``pheno_mcp_router.X``.
"""
...
from pheno_mcp_router.tiers import DEFAULT_REGISTRY, UNKNOWN_PRICING, TierPricing, TierRegistry
...
_CHARS_PER_TOKEN: Final[int] = _config.CHARS_PER_TOKEN
...
# the budget radar.
_DEFAULT_OUTPUT_TOKEN_ESTIMATE: Final[int] = _config.DEFAULT_OUTPUT_TOKEN_ESTIMATE
...
class TokenEstimator:
    ...
    extracts counts from a dict shaped like OpenAI / Anthropic's
    response payload (``{"usage": {"input_tokens": N, "output_tokens": M}}``)
    ...
    def from_response(response: dict[str, Any], message: str) -> TokenUsage:
        """Extract :class:`TokenUsage` from an upstream provider response.
...
class CostCalculator:
    ...
    def estimate_from_response(
        ...
        """Estimate cost from a real upstream provider response.
