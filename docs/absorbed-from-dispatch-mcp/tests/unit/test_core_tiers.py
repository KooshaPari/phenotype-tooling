"""Tests for the :mod:`dispatch_mcp.core.tiers` registry and pricing tables.
...
from dispatch_mcp.core.tiers import (
    DEFAULT_REGISTRY,
    UNKNOWN_PRICING,
    TierPricing,
    TierRegistry,
...
def test_default_registry_contains_all_supported_tiers() -> None:
    ...
    Mirrors ``dispatch_mcp.server.VALID_TIERS`` so a tier added to
    the server but missing from the registry would surface here.
    """
    expected = {
        "worker",
        "main",
