...
import { modelInfoMap } from "../shared/api";
...
router.get("/summary", authenticate, async (req, res) => {
	try {
		...
		if (req.query.projectId)
			dimensions.projectId = req.query.projectId as string;
		...
		if (req.query.organizationId)
			dimensions.organizationId = req.query.organizationId as string;
...
router.get("/detailed", authenticate, async (req, res) => {
	try {
		...
		if (req.query.workflowId)
			dimensions.workflowId = req.query.workflowId as string;
...
router.get("/cost-projection", authenticate, async (req, res) => {
	try {
		...
		if (!modelId || !estimatedTokensIn || !estimatedTokensOut) {
			return res.status(400).json({
				error:
					"modelId, estimatedTokensIn, and estimatedTokensOut are required fields",
			});
		}

		// Get model info from our shared model info map
		const modelInfo = modelInfoMap[modelId as string] || {
			supportsPromptCache: true,
			inputPrice: 3.0, // $3 per million tokens (default if model not found)
