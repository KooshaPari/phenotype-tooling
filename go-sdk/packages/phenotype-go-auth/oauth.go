// Package auth provides shared OAuth utilities for Phenotype services.
package auth
...
type PKCECodes struct {
	// CodeVerifier is the cryptographically random string sent in the token request.
	CodeVerifier string

	// CodeChallenge is the SHA256 hash of the code verifier, sent in the authorization request.
...
// It serves a user-friendly response indicating that authentication was successful.
//
// Parameters:
//   - w: The HTTP response writer
//   - r: The HTTP request
func (s *OAuthServer) handleSuccess(w http.ResponseWriter, r *http.Request) {
	...
	successMsg := "Authentication successful! You can now close this window and return to your CLI."
